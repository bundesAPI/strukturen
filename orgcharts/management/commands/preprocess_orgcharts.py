...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for org_chart in OrgChart.objects.filter(status__in=[OrgChartStatusChoices.NEW]):
            try:
                #yeah its not nice but it does the job
                orgchart_global_id = to_global_id(OrgChartNode.__name__, org_chart.id)
                orgchart_data = requests.get(f"{settings.ML_BACKEND_BASE_URL}/analyze-orgchart/", params={"orgchart_id": orgchart_global_id,
                                                                                          "page": 0})
                print(orgchart_data)
